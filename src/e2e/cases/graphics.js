...
import { disconnect, evaluate, getTargetInfo } from '../../connection.js';
import {
  getOhlcv,
  getPineBoxes,
  getPineLabels,
...
async function defaultChartState() {
  return evaluate(`
    (function() {
      ...
      return {
        symbol: api.symbol(), resolution: api.resolution(), chart_type: api.chartType(),
...
function summaryMatches(summary, raw) {
  return Object.entries(expectedSummary(raw)).every(([key, value]) => Object.is(summary[key], value));
}
...
export function createGraphicsCaseOwner({
  chartState = defaultChartState,
  targetInfo = getTargetInfo,
  getOhlcvImpl = getOhlcv,
  getPineLinesImpl = getPineLines,
  getPineLabelsImpl = getPineLabels,
  getPineBoxesImpl = getPineBoxes,
  getPineTablesImpl = getPineTables,
  disconnectImpl = disconnect,
} = {}) {
  return Object.freeze({
    async run(caseId) {
      if (!GRAPHICS_CASE_IDS.includes(caseId)) return Object.freeze({ status: 'failure', code: 'CASE_FAILED' });
      return runFixedCase(async () => {
        const initialState = await chartState();
        const initialTarget = await targetInfo();
        ...
        try {
          if (caseId === 'graphics_ohlcv_1') {
            const rawBefore = await getOhlcvImpl({ count: 5 });
            const summary = await getOhlcvImpl({ count: 5, summary: true });
            const rawAfter = await getOhlcvImpl({ count: 5 });
            ...
            const lines = await getPineLinesImpl();
            const verboseLines = await getPineLinesImpl({ verbose: true });
            const labels = await getPineLabelsImpl({ max_labels: 50 });
            const verboseLabels = await getPineLabelsImpl({ max_labels: 50, verbose: true });
            const boxes = await getPineBoxesImpl();
            const verboseBoxes = await getPineBoxesImpl({ verbose: true });
            const tables = await getPineTablesImpl();
          ...
          const finalTarget = await targetInfo();
